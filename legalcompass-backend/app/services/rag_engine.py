...
class RAGEngine:
    ...
    def retrieve_chat_context(
        ...
        if is_global:
            logger.info(f"Global risk query detected: '{query}'")
            ...
            if not material_risks:
                logger.info("No materially risky clauses found in candidate pool.")
                return []
            ...
            result = ranked[:k]
            logger.info(f"Ranked {len(result)} material risk clauses: {[c.id for c in result]}")
            return result
